...
【network_interface】 — 외부 통신 인터페이스 (§3.27): Wi-Fi, Bluetooth/BLE, Ethernet, Cellular, Zigbee 등 물리 매체별 1개씩. SCM(보안 통신) 평가의 반복 단위이기도 함.
...
━━━ network_asset vs network_service — 같은 통신을 2개 자산으로 분해 ━━━
하나의 클라우드 통신(MQTTS to broker.example.com:8883)은 아래 둘로 분해해 모두 등록:
  ...
  ※ 데이터 흐름(data_flow)은 더 이상 등록하지 않습니다 — 보안 통신(SCM) 평가는 security_asset·network_asset·network_interface를 직접 대상으로 합니다.

━━━ 작업 절차 (이 순서대로 수행) ━━━

1. **문서 전수 스캔** — 다음 섹션을 빠짐없이 읽어 후보 목록 작성:
   a) 포트·프로토콜 표 → 각 행마다 network_service 1개 + 대부분 network_asset 1개
   b) 네트워크 아키텍처/구성도 → 기기가 제공/사용하는 기능·설정마다 network_asset
...
□ 3. 기기의 통신 인터페이스(Wi-Fi, BLE, Ethernet 등)를 network_interface에 모두 등록했는가? (SCM·NMM·RLM 평가 대상)
