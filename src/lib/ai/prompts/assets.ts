...
━━━ 자산 종류별 역할 구분 (반드시 숙지) ━━━
• network_asset   : 네트워크를 통해 접근 가능한 **자원·엔드포인트** (API 서버, MQTT 브로커, 웹 UI, OTA 서버 등). RLM 평가의 핵심 입력. 네트워크 장비가 아니어도 네트워크에 연결되는 모든 기기에 반드시 존재함.
...
1. **문서의 모든 섹션을 순서대로 검토하세요.**
   ...
   - 네트워크 구성도·아키텍처 → 연결되는 서버·브로커·엔드포인트마다 network_asset 후보
