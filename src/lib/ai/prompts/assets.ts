...
━━━ 자산 종류별 역할 구분 (반드시 숙지) ━━━
• network_asset   : 기기가 네트워크 상에서 **노출·유지·관리하는 자원과 기능** (영상 스트림 엔드포인트, 로컬 웹 관리 UI, 원격 제어 수신 채널, TCP/OCPP 등 프로토콜 연결 자원, 네트워크 파라미터 등). RLM 평가의 핵심 입력. 네트워크에 연결되는 모든 기기에 반드시 존재함. ※ data_flow의 "통신 상대방(외부 서버)"과 혼동 금지 — network_asset은 기기 측 자원임.
• network_service : 기기가 사용하거나 노출하는 **프로토콜·포트·방향** (HTTPS/443, MQTTS/8883 등).
• data_flow       : 기기가 통신하는 **상대방과 목적** (기기→클라우드: 영상 전송, 기기→OTA: 펌웨어 다운로드 등). 포트가 아니라 "누구에게 무엇을" 전달하는지.
이 세 가지는 서로 다른 자산이므로 **모두 각각 등록**해야 합니다.
